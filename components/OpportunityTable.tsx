...
function LegPill({
  ...
  const sideColor = side === "YES" ? "text-emerald-300" : "text-rose-300";
  return (
    <a
      href={url}
      target="_blank"
    ...
    >
      <span className="text-white/60">{venueLabel(venue as never)}</span>
  ...
  if (opportunities.length === 0) {
    return (
      <div className="rounded-xl border border-white/10 bg-white/5 p-8 text-center text-white/60">
        No arbitrage opportunities at or above the configured net margin right now.
      </div>
    );
  }
            ...
            <tr key={opp.id} className="align-top hover:bg-white/[0.03]">
              <td className="px-4 py-4">
                <div className="text-lg font-semibold tabular-nums text-emerald-300">
                  {opp.netMarginPct.toFixed(1)}%
                </div>
                <div className="mt-0.5 text-xs text-white/40 tabular-nums">
                  {opp.grossMarginPct.toFixed(1)}% gross
                </div>
                {opp.netMarginPct >= 25 && (
              ...
              <td className="max-w-md px-4 py-4">
                ...
                <div className="mt-1.5 flex items-center gap-2">
                  ...
                  <span className="text-xs text-white/40">
                    {venueLabel(opp.venues[0])} ↔ {venueLabel(opp.venues[1])}
                  </span>
                </div>
                <div className="mt-1 text-xs text-white/40">
                  {venueLabel(opp.marketB.venue)}: {opp.marketB.title}
                </div>
              </td>
              <td className="px-4 py-4">
